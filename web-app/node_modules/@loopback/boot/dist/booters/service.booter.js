    ...
    async load() {
        ...
        if (this.serviceProviders.length > 0) {
            if (!this.app.serviceProvider) {
                console.warn('app.serviceProvider() function is needed for ServiceBooter. You can add ' +
                    'it to your Application using ServiceMixin from @loopback/service-proxy.');
            }
            else {
                this.serviceProviders.forEach(cls => {
                    this.app.serviceProvider(cls);
                });
            }
        }
    }
...
ServiceBooter = __decorate([
    ...
    __param(2, context_1.inject(`${keys_1.BootBindings.BOOT_OPTIONS}#services`)),
